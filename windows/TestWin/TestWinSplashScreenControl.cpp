...
#if __has_include("TestWinSplashScreenControl.g.cpp")
#include "TestWinSplashScreenControl.g.cpp"
#endif
...
/*
    Based on react-native-splash-screen PR https://github.com/crazycodeboy/react-native-splash-screen/pull/514
    with addition of fade out animation
*/
namespace winrt::TestWin::implementation
{
	Windows::Foundation::IAsyncAction TestWinSplashScreenControl::LoadSplashScreenInformation()
	{
        winrt::apartment_context ui_thread; // Capture ui calling context;

        StorageFile appxmanifestFile = co_await Windows::ApplicationModel::Package::Current().InstalledLocation().GetFileAsync(L"AppxManifest.xml");
	...
	TestWinSplashScreenControl::TestWinSplashScreenControl()
	{
		InitializeComponent();
		Windows::Foundation::IAsyncAction splashScreenInfoAction = LoadSplashScreenInformation();
		splashScreenInfoAction.Completed([](auto, auto) {});
	}

	void TestWinSplashScreenControl::UpdateSplashScreenVisuals(std::string backgroundColor, Uri uri)
	{
        auto actualColor = ParseColor(backgroundColor);
        if (actualColor.has_value())
        {
            SplashBackgroundGrid().Background(SolidColorBrush(actualColor.value()));
        }
        if (uri != nullptr)
        {
    ...
    void TestWinSplashScreenControl::RunFadeOutAnimation()
    {
	...
	std::optional<Windows::UI::Color> TestWinSplashScreenControl::ParseColor(std::string color)
	{
